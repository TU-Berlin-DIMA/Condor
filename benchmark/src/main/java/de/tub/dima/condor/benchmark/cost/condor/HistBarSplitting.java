...
public class HistBarSplitting {
    public static void run(int parallelism, int targetThroughput) throws Exception {
        String jobName = "BarSplittingHistogram Synopses COST test "+parallelism;
        ...
        // Build the synopses
        SingleOutputStreamOperator<WindowedSynopsis<BarSplittingHistogram>> synopsesStream = SynopsisBuilder.build(config);
        final SingleOutputStreamOperator<EquiDepthHistogram> equiDepthHist = synopsesStream.map(new MapFunction<WindowedSynopsis<BarSplittingHistogram>, EquiDepthHistogram>() {
            @Override
            public EquiDepthHistogram map(WindowedSynopsis<BarSplittingHistogram> value) throws Exception {
                return value.getSynopsis().buildEquiDepthHistogram();
            }
        });
