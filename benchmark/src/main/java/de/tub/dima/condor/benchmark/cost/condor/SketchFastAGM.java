...
public class SketchFastAGM {
    public static void run(int parallelism, int targetThroughput, int iteration) throws Exception {
        String jobName = "Synopses COST test FastAGMS | parallelism: "+parallelism + " | iteration: "+iteration+ " | targetThroughput: " + targetThroughput;
        ...
        Class<FastAGMS> synopsisClass = FastAGMS.class;
        Window[] windows = {new TumblingWindow(WindowMeasure.Time, 15000)};
        Object[] synopsisParameters = new Object[]{65536, 5, 42l}; // width, height, seed

        BuildConfiguration config = new BuildConfiguration(inputStream, synopsisClass, windows, synopsisParameters, parallelism);

        // Build the synopses
        SingleOutputStreamOperator<WindowedSynopsis<FastAGMS>> synopsesStream = SynopsisBuilder.build(config);
