...
public class SketchBloomFilter {
    public static void run(int parallelism, int targetThroughput, int iteration) throws Exception {
        String jobName = "Synopses COST test Bloom Filter | parallelism: "+parallelism + " | iteration: "+iteration+ " | targetThroughput: " + targetThroughput;
        ...
        Class<BloomFilter> synopsisClass = BloomFilter.class;
        Window[] windows = {new TumblingWindow(WindowMeasure.Time, 15000)};
        Object[] synopsisParameters = new Object[]{1000, 64, 42l}; // expected number of distinct elements, numBits, seed

        BuildConfiguration config = new BuildConfiguration(inputStream, synopsisClass, windows, synopsisParameters, parallelism);

        // Build the synopses
        SingleOutputStreamOperator<WindowedSynopsis<BloomFilter>> synopsesStream = SynopsisBuilder.build(config);
